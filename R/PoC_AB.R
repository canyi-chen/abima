...
PoC_AB <- function(S, M, Y, X, B = 500, lambda = 2) {
  ...
  med.fit.formula <- as.formula(paste0("M~S+", paste(paste("X", 1:(p - 1), sep = ""), collapse = "+")))
  ...
  PoC_AB_help_function <- function(data, ind) {
    ...
    eps_M_hat <- matrix(resid(med.fit))
    ...
    T_beta_hat <- sqrt(n) * coef(out.fit)['M'] / sigma_beta_M_hat

    T_alpha_hat_ast <- sqrt(n) * coef(med.fit_ast)['S'] / sigma_alpha_S_hat_ast
    T_beta_hat_ast <- sqrt(n) * coef(out.fit_ast)['M'] / sigma_beta_M_hat_ast
    ...
    U_ast <- (coef(med.fit_ast)['S'] * coef(out.fit_ast)['M'] -
      coef(med.fit)['S'] * coef(out.fit)['M']) * (1 - I_alpha_ast * I_beta_ast) +
